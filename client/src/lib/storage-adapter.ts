...
import type { Capture, Bucket, Folder, TaskTemplate, InsertCapture, InsertBucket, InsertFolder, InsertTaskTemplate, Attachment } from '@shared/schema';
    ...
    switch (method) {
      case 'GET':
        if (!param1) {
          ...
          if (!capture) {
            return this.createErrorResponse('Capture not found', 404);
          }
      ...
      case 'POST':
        // POST /api/captures
        if (!data) {
          return this.createErrorResponse('Invalid capture data', 400);
        }
      ...
      case 'PATCH':
        ...
        if (!param1) {
          return this.createErrorResponse('Capture ID required', 400);
        }
        
        const updatedCapture = await indexedDBService.updateCapture(param1, data);
        if (!updatedCapture) {
          return this.createErrorResponse('Capture not found', 404);
        }
        return this.createResponse(updatedCapture);
    ...
    switch (method) {
      case 'GET':
        if (!param1) {
          ...
          if (!bucket) {
            return this.createErrorResponse('Bucket not found', 404);
          }
      ...
      case 'POST':
        if (param1 === 'reorder') {
          ...
          if (!data || !data.orderedIds) {
            return this.createErrorResponse('Invalid reorder data', 400);
          }
          ...
          try {
            ...
            if (error.message.includes('Invalid bucket IDs')) {
          ...
          // POST /api/buckets
          if (!data) {
            return this.createErrorResponse('Invalid bucket data', 400);
          }
          
          const bucketData = { ...data, userId: this.mockUserId };
    ...
    switch (method) {
      case 'GET':
        if (param1 === 'bucket' && param2) {
          ...
          if (!folder) {
            return this.createErrorResponse('Folder not found', 404);
          }
      ...
      case 'POST':
        // POST /api/folders
        if (!data) {
          return this.createErrorResponse('Invalid folder data', 400);
        }
        
        const newFolder = await indexedDBService.createFolder(data);
  ...
  private async handleTaskTemplatesRequest(method: string, urlParts: string[], data?: any): Promise<StorageAdapterResponse> {
    switch (method) {
      ...
      case 'POST':
        ...
        if (!data) {
          return this.createErrorResponse('Invalid template data', 400);
        }
        
        const templateData = { ...data, userId: this.mockUserId };
    ...
    switch (method) {
      case 'GET':
        if (param1 === 'due') {
          ...
          const dueReminders = await indexedDBService.getCapturesDue(this.mockUserId, beforeDate);
          return this.createResponse(dueReminders);
        } else {
          // GET /api/reminders
...
export async function adaptedApiRequest(
  ...
  if (shouldUseLocalStorage()) {
    ...
    // Convert our custom response to a standard Response object
